...
- **Directory Structure Cleanup**:
  - Moved `bin/simple_example.dart` to `example/example.dart` creating a unified, comprehensive example.
  - Moved `bin/integration_test.dart` to `test/integration/bin_test.dart` for better test organization.
  - `bin/` directory now strictly contains only the `ddgs` executable.
- **Improved Documentation**:
  - Rewrote `example/example.dart` to be a professional, fully-commented reference implementation.
  - Added `executables` entry to `pubspec.yaml` for proper `dart pub global activate` support.
- **Test Suite Re-architecture**:
  - Migrated to a structured test directory (`test/unit`, `test/integration`, `test/mocks`, `test/fixtures`).
  - Added `mocktail` for type-safe and efficient mocking.
  - Separated fast unit tests from slow integration tests.
  - Added comprehensive unit tests for `HttpClient`, `SearchResult`, and `DDGS` core logic.
